##' Iterated ensemble Kalman filter (IEnKF2)
##'
...
setGeneric("ienkf2",  function (data, ...)standardGeneric("ienkf2"))

##' @name ienkf2-spatPomp
##' @aliases ienkf2,spatPomp-method
##' @rdname ienkf2
##' @export
setMethod(
  "ienkf2",
  ...
  definition=function (data,
    ...
    tryCatch(
      ...
      error = function (e) pomp:::pStop("ienkf2",conditionMessage(e))
    )
  }
)

ienkf2.internal <- function (object, Nenkf, rw.sd,
...
###################ienkf.filter()##################################
###################################################################
ienkf.filter2 <- function (object, params, Np, enkfiter, rw.sd, cooling.fn,
  ...
  for (nt in seq_len(ntimes)) {
    ...
    parmeans <- rowMeans(params)
    # forecast mean
    ym <- rowMeans(Y)
    # center prediction and forecast ensembles
    params <- params - parmeans
    Y <- Y-ym
    fv <- tcrossprod(Y)/(Np-1)  # forecast variance

    # fv <- tcrossprod(Y)/(Np-1)+R  # forecast variance
    vyx <- tcrossprod(Y,params)/(Np-1)   # forecast/state covariance
    ...
    resid <- y[,nt]-ym
    params <- params+parmeans+crossprod(Kt,resid-Y+Ek)
