#' Measles in UK spatPomp generator for IABF run
#'
...
measles_iabf <- function(U=6,dt=2/365){
  ...
  measles_RPnames <- c("alpha","iota","R0","amplitude","gamma","sigma","mu","sigmaSE","rho","psi","g")
  ...
  measles_rinit <- Csnippet("
    ...
    for (u = 0; u < U; u++) {
      ...
      // compute E[u]. ((βI[u]/pop[u])/σ)*I[u]
      E[u] = nearbyint(((R0*(gamma + mu)*(1-amplitude)*I[u]/m)/sigma)*(float)(I[u]));
      R[u] = pop[u]-S[u]-E[u]-I[u];
      W[u] = 0;
      C[u] = 0;
    }
