  ...
  verbose = getOption("verbose",FALSE))
{
  ...
  cat(file=out,render(define$var.alt,variable="give_log",ptr='__give_log',index=0L))
  cat(file=out,render(define$var.alt,variable="ncovars",ptr='__ncovars',index=0L))
  cat(file=out,render(define$var.alt,variable="t",ptr='__t',index=0L))
  ...
  if (!missing(unit_dmeasure)) {
    registry <- c(registry,"unit_dmeasure")
    cat(file=out,render(header$unit_dmeasure))
    for (v in seq_along(unit_statenames)) {
      cat(file=out,render(define$simplevar,simplevariable=unit_statenames[v],var=paste0("&",unit_statenames[v],"1")))
    }
    for (v in seq_along(obstypes)) {
      cat(file=out,render(define$simpleobs,simpleobservation=obstypes[v],obs=paste0("&",obstypes[v],"1")))
    }
    cat(file=out,callable.decl(unit_dmeasure))
    cat(file=out,unit_dmeasure,footer$unit_dmeasure)
  }
...
define <- list(
  ...
  simplevar="double {%simplevariable%} = *({%var%} + unit);\n",
  simpleobs="double {%simpleobservation%} = *({obs} + unit);",
...
header <- list(
  ...
  unit_dmeasure= "\nvoid __pomp_unit_dmeasure (double *__lik, const double *__y, const double *__x, const double *__p, int *__give_log, const int *__obsindex, const int *__stateindex, const int *__parindex, const int *__covindex, int *__ncovars, const double *__covars, double *__t, int *__unit)\n{\n"
)

fnames <- list(
  unit_dmeasure="__pomp_unit_dmeasure"
)
