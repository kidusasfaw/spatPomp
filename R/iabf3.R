##' Iterated Adapted Bagged Filter 3 (IABF3)
##'
...
h_abf_internal3 <- function (object,
                             params,
                             states,
                             obs_num,
                             prev_meas_weights,
...
iabf_internal3 <- function (object, Nrep, nbhd, Nabf, Np, prop,rw.sd,
  ...
  for (n in seq_len(Nabf)) {
    ...
    for(nt in seq_len(ntimes)){
      ...
      rep_loglik_un <- foreach::foreach(u=seq_len(nunits),
                                        .combine = function(...) abind::abind(..., along=2), # each row is a rep, each col is a unit
                                        .packages=c("pomp", "spatPomp"),
                                        .options.multicore=mcopts) %dopar%
        {
          cond_loglik_u <- vector(length=Nrep)
            rep_filt_weight_un_rep = vapply(mult_rep_output,
              ...
              vapply(mult_rep_output,
                     FUN = function(rep_output) return(rep_output@log_wp_avg[u,]),
                     FUN.VALUE = 1.0)
            cond_loglik_u = rep_filt_weight_un_rep
          cond_loglik_u
        }
      loglik_rep <- apply(rep_loglik_un, MARGIN = 1, FUN = sum)
      rm(rep_loglik_un)
...
setMethod(
  ...
  definition = function (object, Nabf = 1, Nrep, nbhd, Np,prop,
                         rw.sd,
