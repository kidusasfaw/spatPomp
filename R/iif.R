...
iif.internal <- function (object, params, Np,
  ...
  if(missing(vec_dmeasure)||missing(nbhd)||is.na(units)) {
    pfilter.internal(object, params, Np, tol, max.fail,
                     pred.mean, pred.var, filter.mean,
                     filter.traj, cooling, cooling.m,
                     verbose,save.states, save.params,
                     .getnativesymbolinfo
    )
  }
  else {
    ...
    loc.comb.pred.weights = array(data = numeric(0), dim=c(units,ntimes))
