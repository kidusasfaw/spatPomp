...
SEXP do_dmeasure3 (SEXP object, SEXP y, SEXP x, SEXP times, SEXP params, SEXP log, SEXP gnsi)
{
  ...
  SEXP pompfun;
  SEXP cvec, tvec = R_NilValue, retvec;
  ...
  ntimes = length(times);
  if (ntimes < 1)
    errorcall(R_NilValue,"in 'dmeasure3': length('times') = 0, no work to do");
  ...
  if (ntimes != dim[1])
    errorcall(R_NilValue,"in 'dmeasure3': length of 'times' and 2nd dimension of 'y' do not agree");
  ...
  if (ntimes != dim[2])
    errorcall(R_NilValue,"in 'dmeasure3': length of 'times' and 3rd dimension of 'x' do not agree");
  ...
  if ((nreps % nrepsp != 0) || (nreps % nrepsx != 0))
    errorcall(R_NilValue,"in 'dmeasure3': larger number of replicates is not a multiple of smaller");
  ...
  default:
    ...
    errorcall(R_NilValue,"in 'dmeasure3': unrecognized 'mode'"); // # nocov

  break;

  }
  PROTECT(retvec = NEW_NUMERIC(1)); nprotect++;
  REAL(retvec)[0] = 0.1;
  UNPROTECT(nprotect);
  return retvec;
}
